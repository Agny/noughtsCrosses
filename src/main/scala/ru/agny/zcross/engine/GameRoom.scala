...
class GameRoom(id: Int, actorSystem: ActorSystem) {
  ...
  def websocketFlow(user: String): Flow[Message, Message, _] = Flow.fromGraph(GraphDSL.create(source) { implicit b => csource =>
  ...
  def sendMessage(message: Event): Unit = gameRoomActor ! message
}
